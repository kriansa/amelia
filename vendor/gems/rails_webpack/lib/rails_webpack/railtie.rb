...
module RailsWebpack
  ...
  class Railtie < ::Rails::Railtie
    ...
    # This block is needed because `Rails.root` is only defined after the
    # initialization process
    config.after_initialize do |app|
      # This is the path to the manifest file generated by the Webpack
      # counterpart.  You need to match this value with the Webpack settings,
      # otherwise things won't work well.
      app.config.webpack.manifest_file_path =
        Rails.root.join('public', 'assets', '.manifest-webpack.json')

      # This is the path to where the Webpack compile errors are goint to be
      # sent. It's important that the errors are piped into this file so that
      # Rails-Webpack can detect when an error occours at compile-time
      app.config.webpack.error_file_path =
        Rails.root.join('tmp', 'webpack-error.txt')
    end
